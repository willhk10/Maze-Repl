...
It then picks edges at random and removes them if doing so doesn't connect cells which are already connected, which would lead to loops in the maze, meaning multiple paths could lead between the same two points. To efficiently determine whether removing an edge connects two already connected groups of cells, it uses a linked list, which is in the form of a dictionary, to find the root of each set of cells, so if they have the same, it knows not to remove that edge. It repeats this process untill all edges have been picked, at which point the maze is complete.
