...
if isMain:
    ...
    pygame.display.set_caption("Kruskal's Algorithm Maze Creator")
...
def findRoot(node, linkedListDict):
    curNode = node
    while curNode in linkedListDict:
        curNode = linkedListDict[curNode]
    return curNode
...
def generateMaze(perCol, perRow):
    ...
    for y in range(perCol):
        for x in range(perRow):
            if x % 2 == 1 and y % 2 == 1:
                maze[y][x] = 1
                drawNode((y, x), JOINT_COLOR)
                delay(0)
            elif x % 2 == 1 or y % 2 == 1:
                maze[y][x] = 1
                edgeBag.append((y, x))
                drawNode((y, x), EDGE_COLOR)
                delay(0)
    while len(edgeBag) > 0:
